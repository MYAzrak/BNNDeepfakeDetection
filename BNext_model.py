...
stage_out_channel_large = (
    [64] + [128] + [256] * 4 + [512] * 4 + [1024] * 16 + [2048] * 4
)
    ...
    def forward(self, x):
        ...
        if self.temperature < 1e-7:
            ...
            binary_weights_no_grad = (
                real_weights / self.temperature.clamp(min=1e-8)
            ).clamp(-1, 1)
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, drop_rate=0.1, mode="scale"
    ):
...
class BNext(nn.Module):
    def __init__(self, num_classes=1000, size="tiny"):
        ...
        drop_rates = [
            x.item() for x in torch.linspace(0, drop_rate, (len(stage_out_channel)))
        ]
        ...
        for i in range(len(stage_out_channel)):
            if i == 0:
                self.feature.append(
                    firstconv3x3(
                        3, stage_out_channel[i], 1 if num_classes != 1000 else 2
                    )
                )
            elif i == 1:
                self.feature.append(
                ...
                and stage_out_channel[i] != stage_out_channel[1]
            ):
                self.feature.append(
                    basicblock(
                        stage_out_channel[i - 1],
